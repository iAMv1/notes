...
## 1.7 Adversarial Search (Game Playing)

### What is Adversarial Search?

In games where there are **multiple agents** (opponents) with conflicting goals, we need adversarial search techniques. Unlike single-agent search, we must consider that the opponent will try to minimize our success.
...
│                      MIN (Opponent)                              │
│                         ┌─┬─┐                                    │
│                         │ O │  ⬇  Wants MINIMIZE score          │
│                         └─┴─┘                                    │
│                                                                  │
│   MAX tries to get highest score                                │
│   MIN tries to get lowest score                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
**Minimax** is a recursive algorithm used in two-player zero-sum games where:
- **MAX** wants to maximize the score
- **MIN** wants to minimize the score

```python
"""
Minimax Algorithm - Complete Explanation

KEY CONCEPT:
- Two players: MAX and MIN
- MAX maximizes score, MIN minimizes
- Recursively explores all possible moves
- Returns best score assuming optimal opponent

COMPLETENESS: Yes (for finite games)
TIME COMPLEXITY: O(b^m) where b=branching, m=depth
SPACE COMPLEXITY: O(bm)
"""

import math

def minimax(node, depth, is_maximizing, game_tree):
    """
    Minimax Algorithm
    ...
    Args:
        ...
        is_maximizing: True if MAX's turn, False for MIN
        game_tree: Game tree with scores at leaves
    
    Returns:
        Best score achievable from this node
    """
    # BASE CASE: Leaf node (terminal state)
    if is_leaf(node, game_tree):
        return get_utility(node, game_tree)
    
    # RECURSIVE CASE
    if is_maximizing:
        best_score = -math.inf
        for child in get_children(node, game_tree):
            score = minimax(child, depth + 1, False, game_tree)
            best_score = max(best_score, score)
        return best_score
    else:
        best_score = math.inf
        for child in get_children(node, game_tree):
            score = minimax(child, depth + 1, True, game_tree)
...
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   ALPHA: Best value MAX can guarantee at current path          │
│   BETA:  Best value MIN can guarantee at current path          │
│                                                                  │
│   PRUNING RULES:                                                │
│   - If beta ≤ alpha: Prune remaining siblings                  │
│                                                                  │
│   Example Tree:                                                 │
│                                                                  │
│                        MAX                                       │
...
│            2   5    ← β≤α  Pruned                              │
│                                                                  │
│   We don't need to explore MIN's children after seeing 2        │
│   because we already have a better option (3)                  │
└─────────────────────────────────────────────────────────────────┘
```
...
KEY CONCEPT:
- Eliminates unnecessary exploration
- Same result as Minimax, much faster
- Alpha = best value MAX can achieve
- Beta = best value MIN can achieve

TIME COMPLEXITY: O(b^(m/2)) for good ordering
"""

import math

def alpha_beta(node, depth, alpha, beta, is_maximizing, game_tree):
    if is_leaf(node, game_tree):
        return get_utility(node, game_tree)
    ...
    if is_maximizing:
        max_eval = -math.inf
        for child in get_children(node, game_tree):
            eval_score = alpha_beta(child, depth + 1, alpha, beta, 
                                   False, game_tree)
...
| ------------------- | ------- | ------------------- |
| **Time Complexity** | O(b^m)  | O(b^(m/2)) ideal    |
| **Completeness**    | Yes     | Yes                 |
| **Optimality**      | Yes     | Yes                 |
| **Pruning**         | None    | Eliminates branches |
...
- A set of **constraints** (rules about allowed values)
...
│   Variables: X, Y, Z                                            │
│   Domains:   {1, 2, 3}                                          │
│   Constraints: X≠Y, X<Y, X+Y=Z                                 │
│                                                                  │
│   GOAL: Find assignment that satisfies ALL constraints          │
└─────────────────────────────────────────────────────────────────┘
```
...
| --------------- | ------------------- | ------------------ |
| **Unary**       | X ≠ 5               | Single variable    |
| **Binary**      | X ≠ Y               | Two variables      |
| **Global**      | AllDifferent(X,Y,Z) | Multiple variables |
...
```
Problem: Color map with 3 colors (adjacent = different color)
...
```
Problem: Place N queens on N×N chessboard (no attacks)
...
| **LCV**   | Least Constraining Value - choose value that leaves most options |
